...
Before reading or writing recipes, resolve the Hyper state root per `reference/state-root.md`. Every `.hyper/` path in this skill is relative to that root.
...
1. Ensure `.hyper/` is bootstrapped per `reference/bootstrap.md`, then ensure `.hyper/recipes/` exists.
