...
This file is the authoritative instruction set for the **technical-plan** phase when hyper-build routes to it.
...
`reference/data-model.md`. The gate contract is in
`reference/gates.md`.
...
2. **Conflict-triggered re-entry path (writes the revision):**
   ...
   f. Return `awaiting-approval`. Do not delete `plan-conflict.md` here;
      the implement phase deletes it on the next re-entry to `implement`.
...
- When the chosen approach is settled (or when a conflict-revision
  ...
  `dashboard.md` per `reference/dashboard.md` §Decisions log
...
- Do not write subtask files here. That belongs to the execution-plan phase.
...
- Do not delete `plan-conflict.md` from the technical-plan phase. the implement phase deletes it on the next re-entry per its re-entry behavior. Leaving it in place during this phase preserves the audit trail if the revision needs a second review pass.
