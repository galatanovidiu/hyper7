...
This file is the authoritative instruction set for the **verify** phase when hyper-build routes to it.
...
4. Review the diff per `reference/change-review.md` and write the result into
   `checks.md` under `## review`.
...
- When a load-bearing verify ruling is settled (an `Overall: blocked`
  ...
  `reference/dashboard.md` §Decisions log contract, authoring
  as `verify`.
