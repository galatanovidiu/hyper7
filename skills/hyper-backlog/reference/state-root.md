<!-- GENERATED FILE — do not edit here. Source: shared/reference/state-root.md. Regenerate: node scripts/sync-shared.mjs -->
...
Each skill that needs routing state carries its own copy of the probe at its
own base. The probe binary is identical wherever it is vendored. Invocation is
always:
...
## Sub-skill resolution (when the calling skill resolves state vs. reuses it)

Whether a skill calls the probe itself depends on how it was invoked, not on a
fixed list of skill names:

- A skill that runs as an orchestrator or a user-typed entry point calls its
  own local probe at session entry and passes the result down through routing.
- A skill that is dispatched with state already resolved reuses the
  `state_root` (and any other probe fields it needs) passed in its
  conversation context. It does not call the probe again.
- A skill invoked standalone, with no parent that has already resolved state,
  calls its own local `scripts/state.mjs`.

When a skill's SKILL.md says "Resolve the Hyper state root per
`reference/state-root.md`", read it as: *use the `state_root` the calling skill
already resolved through the probe when one is present; otherwise call the
local probe yourself*.
...
| `phase_known` | boolean | `false` when `phase` is not in the `data-model.md` phase enum. |
...
`phase` against the `data-model.md` phase enum:
...
to hyper-build's state-recovery handling. A `cancelled` task left under
